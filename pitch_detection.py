...
def pitch_detection(trace,nfft,threshold_HSI,nr_downsamp_HPS,add_time=1):      
    ### traces
    # trace of day, to scan for harmonic tremor
#     trace = read(path_waveforms+ experiment+ '/'+str(year)+'/'+network+'/' + station + '/' + component[j] + '.D/' + network + '.' + station + '..' + component[j] + '.D.' + str(year) + '.' + '{:03}'.format(int(day)))
...
#     trace += read(path_waveforms_test+'1B.KNR08..BHZ.D.2016.327')
    # trace of day after day of interest
#     trace += read(path_waveforms+ experiment + '/'+str(year)+'/' + network + '/' + station + '/' + component[j] + '.D/' + network + '.' + station + '..' + component[j] + '.D.' + str(year) + '.' + '{:03}'.format(int(day)+1))
    # trace of day before day of interest
#     trace += read(path_waveforms+ experiment + '/'+str(year)+'/' + network + '/' + station + '/' + component[j] + '.D/' + network + '.' + station + '..' + component[j] + '.D.' + str(year) + '.' + '{:03}'.format(int(day)-1))
    # inventory for response remove 
    #
#    inv = read_inventory(inv_path+'RESP.'+network+'.' + station + '..' + component[j]))
